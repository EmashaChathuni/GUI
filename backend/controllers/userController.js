...
export const addUser = async (request, response) => {
  try {
    const { username, email, mnumber, password} = request.body;

    if (!username || !email || !password || !mnumber) {
      return response.status(400).json({
        success : false,
        message: "All fields (username, email, password, mnumber) are required.",
      });
    }
...
export const updateUser = async (request, response) => {
  try {
    ...
    const updatedUser = { username, email, mnumber, password};
